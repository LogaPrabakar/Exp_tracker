...
import { X, Plus, Calendar, User, IndianRupee } from 'lucide-react';

interface AddLoanModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (loan: { name: string; date: string; amount: number; returned: boolean }) => void;
}
...
const AddLoanModal: React.FC<AddLoanModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setFormData({ name: '', date: new Date().toISOString().split('T')[0], amount: '', returned: false });
      setIsLoading(false);
      onClose();
    }, 300);
  };
      ...
      <div className="glass max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-primary-100">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-accent-800">Add New Loan</h2>
          </div>
          <button
            onClick={onClose}
